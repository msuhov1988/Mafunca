...
### The goal is not to cover "everything" about functional programming, but to provide some useful things, namely:
- ### the ability to work with errors without disrupting the program's execution flow
- ### the ability to handle missing values without checking for None
- ### wrap side effects in deferred execution constructs, thus maintaining the purity of functions
...
| catch_map(<br/><nobr>exc_type: type[Union[Exc, TimeoutError]],</nobr><br/><nobr>catcher: Callable[[Union[Exc, TimeoutError]], A]</nobr><br/>)                      | `EffectAsync[A]` | Handler for `Exc` or `TimeoutError` type errors, where `Exc` is a subtype of `Exception`. Wraps the result.          |
| catch_bind(<br/><nobr>exc_type: type[Union[Exc, TimeoutError]],</nobr><br/><nobr>catcher: Callable[[Union[Exc, TimeoutError]], EffectAsync[A]]</nobr><br/>)</nobr> | `EffectAsync[A]` | Handler for `Exc` or `TimeoutError` type errors, where `Exc` is a subtype of `Exception`. Does not wraps the result. |
